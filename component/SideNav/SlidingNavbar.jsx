...
const ExpandableMenu = () => {
  const {setToggleMenu, toggleMenu, setAddSkip, selecteTags, selectedIngredients, selectedCategory} = StateContext()
function skip(){
  ...
  const menuOptions = [
    ...
    { icon: Links(`/recipes-0-id-asc_${selecteTags.map((item) => item.label).join(',')}_${selectedIngredients.map((item) => item.label).join(',')}_${selectedCategory}`, <MdOutlineFastfood />, skip ), name: Links(`recipes`, 'All Recipes') },
    { icon: Links('/favourites',  <AiOutlineHeart />), name: Links('/favourites', 'Favourites') },
    { icon: Links(`/recipes-0-id-asc_${selecteTags.map((item) => item.label).join(',')}_${selectedIngredients.map((item) => item.label).join(',')}_${selectedCategory.value}`,  <AiOutlineSearch />), name: Links('/recipes-0-id-asc', 'Search') },
