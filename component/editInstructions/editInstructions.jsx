...
function EditInstruction({ info }) {
  ...
  async function addItemHandler(e) {
    ...
    try {
      await addItem('/api/editRecipe', {
        recipeId: idRouter, recipeValue: newInstruction, key: instructionKey, stage: '$set',
      });
  ...
  return (
    <form className={classes.form} onSubmit={addItemHandler}>
      <textarea
        ...
        onChange={(e) => { return setNewInstruction(e.target.value); }}
      />
      <div className={classes.buttons}>
        <FormButton text="SAVE" />
        <Button text="CLOSE" color="warning" click={() => { return setEditInstruction(!editInstruction); }} />
      </div>
    </form>
  );
}
 ...
 *
 * the selected option is the set inside {@link setInstructionIndex} which is a global state,
 * that will then be used to target a specific recipe to edit by using its index
 */
  ...
  return (
    <form className={classes.optionsForm}>
      <select ref={option} className={classes.select}>
        {
          instructions.map((_instructions, index) => {
...
export function NewInstruction() {
  ...
  async function addItemHandler(e) {
    ...
    try {
      await addItem('/api/editRecipe', {
        recipeId: idRouter, recipeValue: newInstruction, key: 'instructions', stage: '$push',
      });
  ...
  return (
    <form className={classes.form} onSubmit={addItemHandler}>
      ...
      <div className={classes.buttons}>
        ...
        <Button text="CLOSE" color="warning" click={() => { return setAddInstruction(!addInstruction); }} />
        <FormButton text="ADD" />
      </div>
    </form>
  );
 ...
 *
 * @param {string} apiPath - The URL or path of the API endpoint
 * where the item will be added. eg('/api/filename')
 * @param {Object} item - The item to be added to the API.
 * Should be a JavaScript object.eg({key: value})
 * @returns {Promise<Object>} A promise that resolves to the response data from the API.
 * @throws {Error} If the POST request fails or the response
 * status is not OK, an error is thrown with a message.
 */
